...
import { Upload, Download, FileSpreadsheet, X, ArrowRight, Info, Users } from 'lucide-react';
import { Customer } from '../types';
import { downloadCustomerExcelTemplate, parseCustomersExcelFile, CustomerImportPreview } from '../utils/excelHelpers';

interface CustomersImportModalProps {
  isOpen: boolean;
  onClose: () => void;
  existingCustomers: Customer[];
  onImportComplete: (importedCustomers: Customer[], mode: 'UPSERT' | 'ADD_ONLY' | 'REPLACE') => void;
...
export function CustomersImportModal({
  ...
  const [previewData, setPreviewData] = useState<CustomerImportPreview | null>(null);
  ...
  const handleFileChange = async (file: File) => {
    ...
    try {
      const parsed = await parseCustomersExcelFile(file, existingCustomers);
      setPreviewData(parsed);
    } catch (err: any) {
      showToast(err || 'Gagal membaca file Excel pelanggan', 'error');
  ...
  const handleExecuteImport = () => {
    if (!previewData || previewData.customersToSave.length === 0) {
      showToast('Tidak ada data pelanggan yang dapat diimport', 'error');
      return;
    }

    onImportComplete(previewData.customersToSave, importMode);
    onClose();
  };
  ...
  const filteredDetails = previewData?.details.filter(
    d => d.name.toLowerCase().includes(searchFilter.toLowerCase()) || d.id.toLowerCase().includes(searchFilter.toLowerCase())
  );
      ...
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-2xl flex flex-col overflow-hidden border border-slate-200 shadow-2xl relative">
        ...
        <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900 leading-tight">Import Data Pelanggan via Excel</h3>
              <p className="text-xs text-slate-500">Upload file Excel untuk pembaruan atau pendaftaran massal direktori klien</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-slate-800">
          ...
          <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-emerald-900">Unduh Template Excel Pelanggan</p>
                <p className="text-[11px] text-emerald-700">Gunakan format terstruktur lengkap dengan Nama Perusahaan, Nama Toko / Supermarket / Outlet, Kontak, Alamat, dan NPWP.</p>
              </div>
            </div>
            <button
              onClick={downloadCustomerExcelTemplate}
              className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 whitespace-nowrap transition-colors shadow-2xs"
            >
          ...
          {!previewData && (
            ...
            >
              ...
              />
              <div className="p-4 bg-white rounded-2xl text-emerald-600 mb-3 border border-slate-200 shadow-xs">
                <Upload className="w-8 h-8" />
              </div>
              <p className="text-sm font-bold text-slate-900 mb-1">
                {isProcessing ? 'Memproses data pelanggan dari Excel...' : 'Tarik & Lepas File Excel Pelanggan'}
              </p>
              <p className="text-xs text-slate-500 max-w-sm mb-4">
                Format yang didukung: <span className="text-slate-700 font-mono">.xlsx</span>, <span className="text-slate-700 font-mono">.xls</span>, atau <span className="text-slate-700 font-mono">.csv</span>
              </p>
            ...
            <div className="space-y-5 animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-center">
                  <p className="text-[10px] uppercase font-bold text-emerald-700">Pelanggan Baru</p>
                ...
                <div className="p-3 bg-red-50 border border-red-200 rounded-xl text-center">
                  <p className="text-[10px] uppercase font-bold text-red-700">Diabaikan (Kosong)</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-normal">Perbarui data lama jika ID/Nama cocok, dan tambah pelanggan baru.</p>
                    </div>
                  </label>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-normal">Hanya daftarkan pelanggan baru, abaikan yang sudah terdaftar.</p>
                    </div>
                  </label>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-normal">Hapus master pelanggan lama dan ganti penuh dengan file ini.</p>
                    </div>
                  </label>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <p className="text-xs font-bold text-slate-700">Pratinjau Hasil Impor ({filteredDetails?.length} Pelanggan):</p>
                  <input
                    type="text"
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    <thead className="bg-slate-100 sticky top-0 backdrop-blur-md text-slate-700 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="p-2.5 font-bold">Nama Perusahaan</th>
                        <th className="p-2.5 font-bold">Toko / Outlet</th>
                        <th className="p-2.5 font-bold">Kontak</th>
                        <th className="p-2.5 font-bold">NPWP & Alamat</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 text-slate-700">
                      {filteredDetails?.map((d, i) => (
                        <tr key={i} className="hover:bg-slate-50">
                          ...
                          <td className="p-2.5 font-medium text-emerald-800">{d.storeName || '-'}</td>
                          <td className="p-2.5 text-slate-600">
                            <div>{d.phone || '-'}</div>
                            <div className="text-[10px] text-slate-400">{d.email}</div>
                          </td>
                          <td className="p-2.5 text-slate-500 max-w-xs truncate">
                            {d.npwp && <div className="text-[10px] text-slate-700 font-mono">NPWP: {d.npwp}</div>}
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex justify-between items-center">
          ...
          {previewData && (
            ...
            >
              Proses Impor ({previewData.customersToSave.length} Pelanggan) <ArrowRight className="w-4 h-4" />
            </button>
          )}
        </div>
      </div>
